...
from database.models import Project, Risk, Task, Escalation
...
async def get_risks():
    ...
    try:
        ...
        risks = session.query(Risk).order_by(Risk.id.desc()).all()
...
async def update_task_status(task_id: int, status: str = Query(...)):
    session = SessionLocal()
    try:
